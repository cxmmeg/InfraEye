...
static uint16_t* app_disp_pu16Touch;
...
void app_disp_vInitialize(void)
{
...
#if 0
	i32Return = spi_bus_initialize(VSPI_HOST, &app_touch_sBusCfg, 0);
	ESP_ERROR_CHECK(i32Return);
	...
	ESP_ERROR_CHECK(i32Return);
	ets_printf("------------------------ Device added ------------------------\n");
//	ets_printf("Touch res:0x%04x", i32Return);
//	ESP_ERROR_CHECK(i32Return);
//	app_disp_pu16Touch = heap_caps_malloc(10*sizeof(uint16_t), MALLOC_CAP_DMA);	
//	assert(app_disp_pu16Touch != NULL);
#if 0
//	for(;;)
	{
