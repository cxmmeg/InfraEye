...
static void meshgrid(const double x[62], const double y[46], double xx[2852],
                     double yy[2852]);
                     ...
                     double yy[2852])
{
  ...
  int ibtile;
  int itilerow;
  for (jcol = 0; jcol < 62; jcol++) {
    ibtile = jcol * 46;
    for (itilerow = 0; itilerow < 46; itilerow++) {
  ...
  for (jcol = 0; jcol < 62; jcol++) {
    ibtile = jcol * 46;
    memcpy(&yy[ibtile], &y[0], 46U * sizeof(double));
  }
}
 ...
 * Return Type  : void
 */
void UpscaleImage(const float InputArray[768], float Output[2852])
{
  boolean_T b0;
  boolean_T b1;
  double dv0[62];
  int iy;
  double dv1[46];
  static double Yq[2852];
  static double Xq[2852];
  static float VV[884];
  int ix;
  int k;
  double dv2[32];
  ...
  for (k = 0; k < 2852; k++) {
    if ((Xq[k] >= 0.0) && (Xq[k] <= 62.0) && (Yq[k] >= 0.0) && (Yq[k] <= 46.0))
    {
      if (!b0) {
        for (iy = 0; iy < 32; iy++) {
          dv2[iy] = 2.0 * (double)iy;
        }
      ...
      s = ((float)Xq[k] - 2.0F * ((float)ix - 1.0F)) / 2.0F;
      t = ((float)Yq[k] - 2.0F * ((float)iy - 1.0F)) / 2.0F;
      ss = ((2.0F - s) * s - 1.0F) * s;
